...
.icounts <-inspection_tbl %>% group_by(business, type) %>% summarize(count=n()) %>%  data.frame() %>% spread(type,count, fill=0) %>% select(business, routines =routine, reinspects=reinspection, complaints =complaint) %>% mutate(inspects=routines+reinspects+complaints, retoro=reinspects/routines) %>%  data.frame()
